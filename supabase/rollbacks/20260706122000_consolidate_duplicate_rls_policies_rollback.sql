-- Rollback for 20260706122000_consolidate_duplicate_rls_policies.sql
-- Recreates the pre-consolidation policies verbatim (captured from pg_policies
-- on the hosted project, 2026-07-06).
...
create policy questions_select_student_enrolled on public.questions
  for select to authenticated
  using (
...
create policy choices_select_admin on public.question_choices
  for select to authenticated
  using ((select private.is_admin_user()));
create policy choices_select_student_enrolled on public.question_choices
  for select to authenticated
  using (
...
create policy courses_select_admin on public.courses
  for select to authenticated
  using ((select private.is_admin_user()));
create policy courses_select_student_enrolled on public.courses
  for select to authenticated
  using (
    is_active is true
...
create policy topics_select_admin on public.course_topics
  for select to authenticated
  using ((select private.is_admin_user()));
create policy topics_select_student_enrolled on public.course_topics
  for select to authenticated
  using (
    is_active is true
    and private.can_current_user_access_mcq()
...
create policy profiles_update_self on public.profiles
  for update to authenticated
  using (id = (select auth.uid()) and role = 'student'::app_user_role)
  with check (private.can_update_own_profile(id, role, approved, academic_year, academic_semester, mcq_access_enabled, courses_access_enabled));

-- app_feature_flags
drop policy if exists app_feature_flags_select on public.app_feature_flags;
create policy app_feature_flags_select_admin on public.app_feature_flags
  for select to authenticated
  using ((select private.is_admin_user()));
create policy app_feature_flags_select_public_courses_flag on public.app_feature_flags
  for select to authenticated
  using (feature_key = 'courses_coming_soon'::text);
...
create policy items_write on public.test_block_items
...
create policy items_insert on public.test_block_items
  ...
  with check (
    exists (
      select 1 from public.test_blocks b
      where b.id = test_block_items.block_id
        and (b.user_id = auth.uid() or private.is_admin_user())
    )
  );
