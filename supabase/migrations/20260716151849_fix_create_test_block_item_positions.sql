...
-- version (20260716151251) used 0-based positions and violated
-- test_block_items_position_check. Reassign positions with row_number() so
-- de-duped ids stay gap-free and 1-based. Also revoke anon EXECUTE (Supabase
-- default privileges re-grant it on create; the function rejects anon anyway
-- since auth.uid() is null, but keep the grant surface tight).
