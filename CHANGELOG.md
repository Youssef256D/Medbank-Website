...
### 2026-07-16 — Supabase backend bug fixes (test-history permanence, atomic test blocks, image-URL sanitation, admin RPC grants)
Backend/database only — no served files changed. Five hosted migrations, one per bug, each verified with SQL and re-run against the security/performance advisors.
- **BUG 1 — test results are now permanent.** Removed the 20-day retention that silently ate old results: unscheduled the daily `pg_cron` job (was `17 2 * * *` calling `delete_old_test_history_entries(20)`), and dropped the `trg_test_history_entries_retention_window` trigger + its `enforce_test_history_retention_window()` function (it returned NULL for any insert/update with `completed_at` older than 20 days). Verified by inserting a 400-day-backdated `test_history_entries` row inside a rolled-back transaction — it now survives. The pruning function is kept but unscheduled (available for future opt-in payload compaction). Table shape / constraints / unique index on `(user_id, external_id)` untouched. Migration `20260716151050_make_test_history_permanent.sql`.
- **BUG 2 — phantom (item-less) test_blocks.** 2489 of 2647 blocks had zero items (all historical, newest 2026-04-05) from a non-atomic client write. Added `create_test_block_with_items(...)` — **SECURITY INVOKER** so existing RLS enforces ownership unchanged (`user_id = auth.uid()` + `private.can_current_user_access_mcq()`) — which inserts the block + items + initial state in one atomic function body, de-dupes question ids, assigns 1-based contiguous positions, and is idempotent on `external_id` for safe retries (mobile app included). Verified: happy path, dedupe, unauthenticated rejection, and atomic rollback on a bad question id (no orphan block). Existing phantom rows were **left as-is** per owner decision. Migrations `20260716151251_add_create_test_block_with_items_rpc.sql` + `20260716151849_fix_create_test_block_item_positions.sql` (position fix).
- **BUG 3 — junk image columns.** All non-null `questions.question_image_url` (4) and `explanation_image_url` (234) values were pasted CSV row text, not URLs. NULLed every value not matching `^https?://` and added validated CHECK constraints so only NULL or http(s) can be stored going forward. Verified 0 invalid remain and the constraints reject a bad value. Migration `20260716152031_sanitize_question_image_urls.sql`.
- **BUG 4 — admin summary RPC grants.** Kept `authenticated` EXECUTE on `get_admin_question_count_summary()` (owner-confirmed): the admin dashboard calls it from the browser as `authenticated` and the function already self-gates on `profiles.role = 'admin'`; revoking would break the dashboard. Revoked PUBLIC + anon as defense-in-depth. The remaining advisor lint for it is an accepted, by-design finding. Migration `20260716152106_harden_admin_question_count_summary_grants.sql`.
- **Manual follow-up for the owner:** enable **leaked-password protection** (HaveIBeenPwned) in the Supabase dashboard — Authentication → Providers/Policies → Password settings. It cannot be toggled via SQL/MCP.
- No RLS was weakened/disabled; the access-control functions (`private.can_current_user_access_question` etc.) were not touched. No `app-version` bump (no served file changed).
